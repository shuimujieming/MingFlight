...
kalman gx_kalman,gy_kalman,gz_kalman;
float mpu_kalman_gx,mpu_kalman_gy,mpu_kalman_gz;
void BMI088_Offset_Cali()
{
	
	kalmanCreate(&gx_kalman,0.005,0.01);
	kalmanCreate(&gy_kalman,0.005,0.01);
	kalmanCreate(&gz_kalman,0.005,0.01);
...
int gBuf[3] = {0};
int aBuf[3] = {0};
float low_filter_coefficient = 0.3;
...
void BMI088_Get_Data()
{
	...
	//µÍÍ¨ÂË²¨
	mpu_data.ax = aBuf[0] = aBuf[0] * (1 - low_filter_coefficient) + mpu_data.ax * low_filter_coefficient;
	mpu_data.ay = aBuf[1] = aBuf[1] * (1 - low_filter_coefficient) + mpu_data.ay * low_filter_coefficient;
	mpu_data.az = aBuf[2] = aBuf[2] * (1 - low_filter_coefficient) + mpu_data.az * low_filter_coefficient;
	
	mpu_data.gx = gBuf[0] = gBuf[0] * (1 - low_filter_coefficient) + mpu_data.gx * low_filter_coefficient;
	mpu_data.gy = gBuf[1] = gBuf[1] * (1 - low_filter_coefficient) + mpu_data.gy * low_filter_coefficient;
	mpu_data.gz = gBuf[2] = gBuf[2] * (1 - low_filter_coefficient) + mpu_data.gz * low_filter_coefficient;
	
	//¿¨¶ûÂüÂË²¨
	mpu_data.gx = KalmanFilter(&gx_kalman,mpu_data.gx);
	mpu_data.gy = KalmanFilter(&gy_kalman,mpu_data.gy);
	mpu_data.gz = KalmanFilter(&gz_kalman,mpu_data.gz);
